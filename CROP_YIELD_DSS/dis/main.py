    ...
    try:
        ...
        for f in features:
            ...
            db.add(AuxiliaryData(
                ...
                precip_sum=0.0,
                temp_mean=0.0,
                et_mean=0.0,      # FIXED: Matches updated models.py
                elevation_m=0.0,
                soil_texture=0.0,
                yield_2020=0.0
            ))
...
async def ingest_csv(
    ...
    RECALIBRATED: Performs an UPSERT to merge stats with existing GeoJSON shapes.
    """
    content = await file.read()
    ...
    try:
        if table_type == "wards":
            ...
            for _, row in df.iterrows():
                ...
                w_year = int(row.get('year') or 2024)
                ...
                u = cast(Any, existing_unit) if existing_unit else None
                if u:
                    # UPDATE EXISTING RECORD
                    u.ndvi_mean = float(row.get('ndvi_mean') or row.get('ndvi') or 0.0)
                    u.precip_sum = float(row.get('precip_total') or row.get('precip_sum') or 0.0)
                    u.gdd_total = float(row.get('gdd_total') or 0.0)
                    u.et_mean = float(row.get('et_mean') or row.get('et') or 0.0) # FIXED: Added to update logic
                    u.yield_2020 = float(row.get('potential_yield') or row.get('yield_2020') or 0.0)
                    u.elevation_m = float(row.get('elevation_mean') or row.get('elevation') or 0.0)
                    u.soil_texture = float(row.get('soil_texture') or row.get('soil') or 0.0)
                    
                    if row.get('county_name'):
                        u.county_name = str(row.get('county_name'))
                else:
                    # INSERT NEW RECORD
                    db.add(AuxiliaryData(
                        ...
                        precip_sum=float(row.get('precip_total', 0.0)),
                        gdd_total=float(row.get('gdd_total', 0.0)),
                        et_mean=float(row.get('et_mean', 0.0)), # FIXED: Added to insert logic
                        yield_2020=float(row.get('potential_yield', 0.0)),
            ...
            for _, row in df.iterrows():
                ...
                db.add(YieldObservation(
                    ...
                    precip_sum=row.get('precip_total'), # standardized to sum/total
                    temp_mean=row.get('temp'), 
                    et_mean=row.get('et_mean', 0.0),    # FIXED: Added to ML samples
