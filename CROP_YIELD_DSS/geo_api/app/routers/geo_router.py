...
def get_dashboard_summary(db: Session = Depends(get_db)):
    """
    Aggregates empirical statistics for the Dashboard StatCards.
    Replaces mock numbers with real PostGIS and SPAM baseline aggregates.
    """
    try:
        total_wards = db.query(models.AuxiliaryData.ward_id).distinct().count()
        # Average of the SPAM 2020 baseline across the country
        avg_spam = db.query(func.avg(models.AuxiliaryData.yield_2020)).scalar() or 0.0
        total_rasters = db.query(models.RasterAsset).count()
        
        return {
            "totalWards": total_wards,
            "avgHistoricalYield": round(float(avg_spam), 2),
            "totalAssets": total_rasters,
            "systemHealth": "Operational"
        }
    except Exception as e:
        logger.error(f"Dashboard summary failed: {e}")
        return {"totalWards": 0, "avgHistoricalYield": 0.0, "totalAssets": 0, "systemHealth": "Error"}

@router.get("/dashboard/chart")
def get_chart_data(db: Session = Depends(get_db)):
    """
    Calculates data for the Yield Gap Analysis chart.
    Compares SPAM 2020 Baselines vs. Real-time System Predictions.
    """
    try:
        # Fetch wards that have real SPAM data
        wards = db.query(models.AuxiliaryData).filter(models.AuxiliaryData.yield_2020 > 0).limit(6).all()
        
        labels = [w.ward_name for w in wards]
        baseline_data = [round(float(cast(Any, w.yield_2020) or 0), 2) for w in wards]
        
        prediction_data = []
        for w in wards:
            # Find the average of system predictions for this specific county unit
            avg_p = db.query(func.avg(models.YieldObservation.yield_value)).filter(
                models.YieldObservation.county_name == w.county_name
            ).scalar()
            
            # If no prediction has been run for this unit yet, we show 0
            prediction_data.append(round(float(avg_p), 2) if avg_p else 0.0)

        return {
            "labels": labels,
            "datasets": [
                {"label": "Baseline (SPAM 2020)", "data": baseline_data},
                {"label": "Prediction (2024 Hybrid)", "data": prediction_data}
            ]
        }
    except Exception as e:
    ...
    return {
        ...
        "biophysical_signature": {
            "NDVI (Biomass)": {
                ...
                "desc": "Photosynthetic vigor vs County mean."
            },
            "Precipitation (mm)": {
                "val": round(float(cast(Any, unit.precip_sum) or 0), 2),
                "dev": None,
                "desc": "Total seasonal rainfall (CHIRPS)."
            },
            "Heat Sum (GDD)": {
                "val": round(float(cast(Any, unit.gdd_total) or 0), 0),
                "dev": None,
                "desc": "Thermal energy accumulation (ERA5)."
            },
            "2020 Baseline": {
                "val": round(float(cast(Any, unit.yield_2020) or 0), 2),
                "dev": None,
                "desc": "Historical SPAM 2020 yield potential."
            }
        }
    }
...
def query_point(request: QueryPointRequest, db: Session = Depends(get_db)):
    ...
    if stack_asset:
        try:
            ...
            if aux_results and len(aux_results) > 0:
                ...
                features_dict.update({
                    ...
                    'gdd_total': float(cast(Any, getattr(aux_data, 'gdd_total', 1400.0))),
                    'precip_total': float(cast(Any, getattr(aux_data, 'precip_sum', 600.0)))
                })
        except Exception as e:
        ...
        features_dict = {
            "ndvi_mean": 0.52, "precip_total": 600.0, "gdd_total": 1400.0, 
            "elevation_mean": 1850.0, "soil_texture": 2.0, "temp_mean": 21.5
        }

    features_list = [Feature(name=str(k), value=float(v)) for k, v in features_dict.items() if isinstance(v, (int, float))]
